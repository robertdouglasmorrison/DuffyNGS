# protienConstructPeptidePileups.R -- show evidence that raw read peptide land on a protein construct
...
proteinConstructPileupSummary <- function( constructSaveFile, sampleID, geneName="Var2csa", 
	...
	pctAA <- bestAA <- strAApcts <- depth <- nAAcalls <- vector( length=nx)
	for ( i in 1:nx) {
		myAAs <- aaCalls[ , i]
		myWts <- aaWeights[ , i]
		drops <- which( myAAs == "")
		...
		if ( length(myAAs)) {
			...
			allPcts <- myFreqs * 100 / sum(myFreqs)
			pctAA[i] <- allPcts[1]
			allPcts <- round( allPcts)
			allPcts <- allPcts[ allPcts > 0]
			nAAcalls[i] <- length(allPcts)
	...
	sumTbl <- data.frame( "Position"=1:nAAout, "ConsensusAA"=bestAA[1:nAAout], "ReadDepth"=depth[1:nAAout],
				"Percentages"=strAApcts[1:nAAout], "Domain"=domainTxt[1:nAAout], stringsAsFactors=FALSE)
	...
	return( invisible( list("Discrepencies"=badAAlocs, "FinalConsensusPercentage"=finalPct, "PctPerfect"=pctPerfect, 
			"AvgNonperfectPercent"=avgNonperfectPercent, "AvgVariants"=avgNvariants)))
}
