...
`pipe.CellTypeProportions` <- function( sampleID, annotationFile="Annotation.txt", optionsFile="Options.txt", 
	...
	if (recalculate != "none" || is.null(cellAns)) {
		...
		if ( (recalculate %in% c("all","profile")) || (recalculate == "missing" && all(is.na(pcts1)))) {
		...
		if ( (recalculate %in% c("all","profile")) || (recalculate == "missing" && all(is.na(pcts2)))) {
		...
		if ( (recalculate %in% c("all","profile")) || (recalculate == "missing" && all(is.na(pcts3)))) {
		...
		if ( (recalculate %in% c("all","deconvolution")) || (recalculate == "missing" && all(is.na(pcts4)))) {
			...
			ans4 <- fileSet.TranscriptDeconvolution( files=transcriptFile, fids=sampleID, algorithm="port",
						useLog=FALSE, plot=TRUE, plot.path=celltype.path, plot.col=cellTypeColors, verbose=F)
			if ( ! is.null(ans4)) pcts4 <- ans4$BestFit
		}
		
		# 5)  Transcriptome Deconvolution, by NLS using the 'GenSA' simulated annealing algorithm...
		if ( (recalculate %in% c("all","deconvolution")) || (recalculate == "missing" && all(is.na(pcts5)))) {
		...
		if ( (recalculate %in% c("all","deconvolution")) || (recalculate == "missing" && all(is.na(pcts6)))) {
			...
			#				useLog=TRUE, plot=TRUE, plot.path=celltype.path, plot.col=cellTypeColors, verbose=F)
			#if ( ! is.null(ans6)) pcts6 <- ans6$BestFit
		}
		
		# 7)  Transcriptome Deconvolution, by NLS using the 'GenSA' simulated annealing algorithm...
		if ( (recalculate %in% c("all","deconvolution")) || (recalculate == "missing" && all(is.na(pcts7)))) {
		...
		if ( (recalculate %in% c("all","deconvolution")) || (recalculate == "missing" && all(is.na(pcts8)))) {
