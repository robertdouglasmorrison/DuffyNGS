...
bam_index_t *bam_index_load_core(FILE *fp)
{
	...
	size_t nbytes = fread(magic, 1, 4, fp);
	...
	for (i = 0; i < idx->n; ++i) {
		...
		// load linear index
		nbytes = fread(&index2->n, 4, 1, fp);
		if (bam_is_be) bam_swap_endian_4p(&index2->n);
		index2->m = index2->n;
		index2->offset = (uint64_t*)calloc(index2->m, 8);
		nbytes = fread(index2->offset, index2->n, 8, fp);
		if (bam_is_be)
