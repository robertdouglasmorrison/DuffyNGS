...
SEXP bam_range_get_align_df(SEXP pRange)
{
	...
	// Column 9: mate RefID
	SEXP mref_vector;
	PROTECT(mref_vector=allocVector(INTSXP,nRows));
	++nProtected;

	// Column 10: mate Position
	SEXP mpos_vector;
	PROTECT(mpos_vector=allocVector(INTSXP,nRows));
	++nProtected;


	// Column 11: insert size
	SEXP isize_vector;
	...
	for(i=0;i<nRows;++i)
	{
		...
		INTEGER(mref_vector)[i]=(align->core.mtid);
		INTEGER(mpos_vector)[i]=(align->core.mpos) + BASE_1;
	...
	SET_VECTOR_ELT(dflist,9,mref_vector);
	SET_VECTOR_ELT(dflist,10,mpos_vector);
	SET_VECTOR_ELT(dflist,11,isize_vector);
	...
	SET_STRING_ELT(col_names,9,mkChar("mrefid"));
	SET_STRING_ELT(col_names,10,mkChar("mposition"));
	SET_STRING_ELT(col_names,11,mkChar("insertsize"));
	setAttrib(dflist,R_NamesSymbol,col_names);
...
{
	if(TYPEOF(pRange)!=EXTPTRSXP)
		error("[bam_range_get_mate_refid] No external pointer!");
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	SEXP mateRefIDans;
	PROTECT(mateRefIDans=allocVector(INTSXP,nRows));

	for(i=0;i<nRows;++i)
	{
		align=get_const_next_align(l);
		INTEGER(mateRefIDans)[i]=align->core.mtid;
	}
	// Reset curr_el pointer
	l->curr_el=e;
...
SEXP bam_range_get_mate_position(SEXP pRange)
{
	if(TYPEOF(pRange)!=EXTPTRSXP)
		error("[bam_range_get_mate_position] No external pointer!");
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	SEXP matePositionans;
	PROTECT(matePositionans=allocVector(INTSXP,nRows));

	for(i=0;i<nRows;++i)
	{
		align=get_const_next_align(l);
		INTEGER(matePositionans)[i]=(align->core.mpos) + BASE_1;
	}
	// Reset curr_el pointer
	l->curr_el=e;

	UNPROTECT(1);
	return matePositionans;
}
...
SEXP bam_range_get_align_qualities(SEXP pRange)
{
	...
	{
		align=get_const_next_align(l);
		raw_qual= (unsigned char*) bam1_qual(align);
...
SEXP bam_range_mate_strand_reverse(SEXP pRange)
{
	if(TYPEOF(pRange)!=EXTPTRSXP) {
		error("[bam_range_mate_strand_reverse] No external pointer!");
		return R_NilValue;
	}
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	{
		align=get_const_next_align(l);
		LOGICAL(boolans)[i]= bam1_mstrand( align);
	}
	// Reset curr_el pointer
	l->curr_el=e;
...
SEXP bam_range_is_paired(SEXP pRange)
{
	if(TYPEOF(pRange)!=EXTPTRSXP)
		error("[bam_range_is_pair] No external pointer!");
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	{
		align=get_const_next_align(l);
		LOGICAL(boolans)[i]= ((align->core.flag & BAM_FPAIRED) != 0);
	}
	// Reset curr_el pointer
	l->curr_el=e;
...
SEXP bam_range_get_cigar_string(SEXP pRange)
{
	if(TYPEOF(pRange)!=EXTPTRSXP)
		error("[bam_range_get_cigar_string] No external pointer!");
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	for(i=0;i<nRows;++i)
	{
		...
		if(cigar2str(buf,align)==0)
		{
			error("[bam_range_get_cigar_string] Cigar error!\n");
			return R_NilValue;
		}
		SET_STRING_ELT(cigarAns,i,mkChar(buf));
		clear_buf(buf,buf_size);
		/////////////////////////////////////////
	}
	// Reset curr_el pointer
...
{
	if(TYPEOF(pRange)!=EXTPTRSXP)
		error("[bam_range_mate_is_unmapped] No external pointer!");
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	...
	{
		align=get_const_next_align(l);
		LOGICAL(boolans)[i]= ((align->core.flag & BAM_FMUNMAP) != 0);
	}
	// Reset curr_el pointer
	l->curr_el=e;
...
SEXP bam_range_get_cigar_df(SEXP pRange, SEXP readUnits)
{
	...
	for(k=0;k<nRows;++k)
	{
		...
		for(i=0;i<nRows;++i)
		{
			...
			if(nout>=nOut)
			{
				error("[bam_range_getCigar_df] Allocation exhausted.  recompile with larger 'MAX_CIGAR_PER_ALIGN'");
				return R_NilValue;
			}
		}
	}
...
SEXP bam_range_modify(SEXP pRange, SEXP RefID, SEXP Pos, SEXP MateRefID, SEXP MatePos, 
				SEXP ReadID, SEXP Seq, SEXP Qual)
{
	...
	align_list *l=(align_list*)(R_ExternalPtrAddr(pRange));
	// Read Address of current align->for reconstitution at the end
	// not used...    align_element *e=l->curr_el;
	wind_back(l);
	...
	if ( TYPEOF(MateRefID) == INTSXP) {
		mrefid=INTEGER(MateRefID);
	}
	if ( TYPEOF(MatePos) == INTSXP) {
		mpos=INTEGER(MatePos);
	}
	...
	for(i=0;i<nRows;++i)
	{
		...
		if ( TYPEOF(MateRefID) == INTSXP) {
			int newMateRefID = mrefid[i];
			newAlign->core.mtid=newMateRefID;
		}
		if ( TYPEOF(MatePos) == INTSXP) {
			int newMatePos = mpos[i];
			newAlign->core.mpos=newMatePos - BASE_1;
		}
...
SEXP bam_range_get_mismatch_df(SEXP pRange, SEXP readUnits)
{
	...
	if(nout>=nOut)
	{
		error("[bam_range_get_mismatch_df] Allocation exhausted.  Rrecompile with larger 'MAX_CIGAR_PER_ALIGN'");
		return R_NilValue;
	}
	align=get_const_next_align(l);
...
SEXP bam_align_modify(SEXP pAlign, SEXP RefID, SEXP Pos, SEXP MateRefID, SEXP MatePos, 
				SEXP ReadID, SEXP Seq, SEXP Qual)
{
	if(TYPEOF(pAlign)!=EXTPTRSXP) {
	...
	if ( TYPEOF(MateRefID) == INTSXP || TYPEOF(MateRefID) == REALSXP) {
		int newMateRefID = *INTEGER( AS_INTEGER( MateRefID));
		newAlign->core.mtid=newMateRefID;
	}
	if ( TYPEOF(MatePos) == INTSXP || TYPEOF(MatePos) == REALSXP) {
		int newMatePos = *INTEGER( AS_INTEGER( MatePos));
		newAlign->core.mpos=newMatePos - BASE_1;
	}
...
SEXP bam_align_get_align_qualities(SEXP pAlign)
{
	...
	char *raw_qual= (char*) bam1_qual(align);
...
SEXP bam_align_mate_is_unmapped(SEXP pAlign)
{
	...
	SEXP ans;
	PROTECT(ans=allocVector(LGLSXP,1));
	LOGICAL(ans)[0]=align->core.flag & BAM_FMUNMAP;
	UNPROTECT(1);
	return ans;
}
...
SEXP bam_align_get_cigar_string(SEXP pAlign)
{
	if(TYPEOF(pAlign)!=EXTPTRSXP)
	...
	if(cigar2str(buf,align)==0)
	{
		error("[bam_align_get_cigar_string] Cigar error!\n");
		return R_NilValue;
	}
